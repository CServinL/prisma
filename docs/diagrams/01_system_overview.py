...
ov.group("Server",     color="#0ea5e9", label="prisma serve (:8765)")
...
ov.add_component("fastapi",      label="FastAPI app",       layer="api",       group="Server",    tech="Python 3.14")
ov.add_component("ui_static",    label="Static /app",       layer="api",       group="Server",    tech="ui/build/ → StaticFiles")
ov.add_component("ui_watcher",   label="UI watcher",        layer="api",       group="Server",    tech="mtime poll → npm build")
...
ov.add_component("graphify_svc", label="GraphifyService",   layer="services",  group="Services",  tech="Watchdog + Ollama")
ov.add_component("chroma_svc",   label="ChromaService",     layer="services",  group="Services",  tech="Watchdog + embeddings")
...
ov.add_component("chromadb",     label="ChromaDB",          layer="storage",   group="Local",     tech="vault/chromadb/")
...
ov.connect("tauri_shell",  "fastapi",      label="HTTP :8765")
ov.connect("pwa_browser",  "fastapi",      label="HTTP :8765")
ov.connect("fastapi",      "ui_static",    label="mounts /app")
ov.connect("fastapi",      "ui_watcher",   label="daemon thread")
...
ov.connect("vault_svc",    "vault_fs",     label="read / write")
ov.connect("graphify_svc", "graphify_out", label="read / write")
ov.connect("chroma_svc",   "chromadb",     label="upsert / query")
ov.connect("zotero_svc",   "pending_q",    label="queue writes (offline)")
ov.connect("ui_static",    "ui_build",     label="serves")
ov.connect("graphify_svc", "ollama",       label="graph extraction")
ov.connect("chroma_svc",   "ollama",       label="nomic-embed-text")
ov.connect("zotero_svc",   "zotero_local", label="reads (offline-first)")
ov.connect("zotero_svc",   "zotero_web",   label="reads + writes")
ov.connect("stream_mgr",   "search_apis",  label="paper search")
ov.connect("stream_mgr",   "zotero_svc",   label="save results")
...
sl.connect("api_router",   "graphify_svc", label="taint / status")
sl.connect("api_router",   "chroma_svc",   label="deep search")
sl.connect("api_router",   "hybrid_client",label="Zotero endpoints")
...
sl.connect("hybrid_client","pending_q",    label="offline writes")
sl.connect("hybrid_client","zotero_local", label="reads")
sl.connect("hybrid_client","zotero_web",   label="reads + writes")
...
ix.connect("vault_watchdog", "graphify_svc",   label="file changed")
ix.connect("vault_watchdog", "chroma_svc",     label="file changed")
ix.connect("startup_scan",   "graphify_svc",   label="full scan")
ix.connect("startup_scan",   "chroma_svc",     label="full scan")
ix.connect("graphify_svc",   "graph_prompt",   label="md content")
ix.connect("graph_prompt",   "ollama_graph",   label="extract entities/relations", style="dashed")
ix.connect("ollama_graph",   "graph_prompt",   label="JSON result", style="dashed")
ix.connect("graph_prompt",   "graphify_store", label="persist nodes")
ix.connect("chroma_svc",     "chunk_split",    label="md content")
ix.connect("chunk_split",    "ollama_embed",   label="chunks to embed", style="dashed")
