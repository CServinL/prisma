...
ov.add_component("kg_out", label="Knowledge graph (Kùzu)", layer="storage",   group="Local",     tech="vault/kg-out/")
...
sl.add_component("kg_svc", label="KnowledgeGraphService", layer="knowledge", group="Knowledge",  tech="watchdog → per-section Ollama extraction")
...
sl.connect("api_router",   "kg_svc", label="taint")
sl.connect("api_router",   "chroma_svc",   label="search")
sl.connect("api_router",   "zotero_client",label="zotero")
...
sl.connect("kg_svc", "kg_out")
sl.connect("chroma_svc",   "chromadb")
sl.connect("zotero_agent", "zotero_client")
sl.connect("zotero_client","pending_q",    label="offline")
sl.connect("zotero_client","zotero_web",   label="r/w")
...
ix.group("KG",   color="#10b981", label="Knowledge graph pipeline")
...
ix.add_component("kg_svc",   label="KnowledgeGraphService",  layer="kg",  group="KG", tech="queue changed files")
ix.add_component("graph_prompt",   label="Graph extractor",      layer="kg",  group="KG", tech="entity/relation prompt, per-section (semchunk)")
ix.add_component("kg_store", label="Knowledge graph (Kùzu)",layer="kg",  group="KG", tech="kg-out/ (embedded graph DB)")
...
ix.add_component("ollama_graph",   label="Ollama (graph)",      layer="llm",       group="LLM",      tech="prisma-kg:7b")
ix.add_component("ollama_embed",   label="Ollama (embed)",      layer="llm",       group="LLM",      tech="nomic-embed-text")

ix.connect("vault_watchdog", "kg_svc",   label="changed")
ix.connect("vault_watchdog", "chroma_svc",     label="changed")
ix.connect("startup_scan",   "kg_svc",   label="scan")
ix.connect("startup_scan",   "chroma_svc",     label="scan")
ix.connect("kg_svc",   "graph_prompt",   label="content")
...
ix.connect("graph_prompt",   "kg_store", label="persist")
