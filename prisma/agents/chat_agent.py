...
# ToolSpec.grounding: RECALL (this session's own turns) and THINK (looks
# nothing up) are not grounding; a zotero:<key> source counts as much as a
# vault slug. Each grounding tool returns empty when it has nothing citable,
# so an empty result reads as ungrounded.
