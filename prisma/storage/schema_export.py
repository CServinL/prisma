...
def _drop_path_from_required(schema: dict) -> dict:
    ...
    (Note/Source/Chat/Stream) because Pydantic sees no default for it, but
    it's never actually part of a stored file's own content -- every render
    function (_render_frontmatter's hand-built `fm` dict, vault.py's
    save_chat_session's explicit exclude set) derives it from the file's
    location on load instead. Validating a real on-disk file against the
    un-adjusted schema would fail on a field that's correctly, deliberately
    absent -- this makes the exported schema describe the actual stored
    shape, not the fuller in-memory one."""
