...
Lives in prisma.server.web_app, not prisma.server.app — the Web process owns
UI serving and the dev watcher independently of the API process (ADR-012).
"""
import threading
import time
...
def test_src_hash_empty_dir(tmp_path):
    ...
    from prisma.server.web_app import _src_hash
    with patch("prisma.server.web_app._ui_src", tmp_path):
        h1 = _src_hash()
        h2 = _src_hash()
    assert h1 == h2
...
def test_watcher_increments_version_on_change(tmp_path):
    ...
    """
    import prisma.server.web_app as web_module

    original_state = web_module._ui_dev_state.copy()
    try:
        web_module._ui_dev_state = {"version": 0, "building": False}
        ...
        with patch("prisma.server.web_app._src_hash", side_effect=hash_sequence), \
             patch("prisma.server.web_app.subprocess.run", side_effect=fake_run), \
             patch("prisma.server.web_app.time.sleep", side_effect=controlled_sleep):
            try:
                web_module._ui_watcher()
            except StopIteration:
                pass

        assert build_called.is_set()
        assert web_module._ui_dev_state["version"] == 1
        assert web_module._ui_dev_state["building"] is False

    finally:
        web_module._ui_dev_state = original_state
...
def test_watcher_no_build_when_unchanged(tmp_path):
    ...
    original_src = web_module._ui_src
    original_state = web_module._ui_dev_state.copy()

    try:
        web_module._ui_src = tmp_path
        web_module._ui_dev_state = {"version": 0, "building": False}
        ...
        with patch("prisma.server.web_app.subprocess.run") as mock_build, \
        ...
        assert web_module._ui_dev_state["version"] == 0

    finally:
        web_module._ui_src = original_src
        web_module._ui_dev_state = original_state
...
def test_dev_version_endpoint_reflects_state():
    ...
    original_state = web_module._ui_dev_state.copy()
    try:
        web_module._ui_dev_state = {"version": 3, "building": True}
        client = TestClient(web_module.app, raise_server_exceptions=True)
        r = client.get("/ui/dev/version")
        assert r.status_code == 200
        data = r.json()
